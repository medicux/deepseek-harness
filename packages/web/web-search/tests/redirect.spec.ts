 ...
 * request-init assertions alone cannot observe that boundary. Ported for the native
 * Anthropic-compatible backend; every provider sets the same `redirect: 'error'`.
 */

import { afterAll, beforeAll, describe, expect, it } from 'vitest'
import { createServer, type IncomingMessage, type Server } from 'node:http'
import type { AddressInfo } from 'node:net'
import { AnthropicNativeSearchProvider, type AnthropicNativeSearchOptions } from '../src/native-anthropic.ts'

const searchProvider = (options: AnthropicNativeSearchOptions): AnthropicNativeSearchProvider =>
  new AnthropicNativeSearchProvider('deepseek', 'DeepSeek', () => options)
...
describe('native redirect policy', () => {
