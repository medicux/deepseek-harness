...
import { EXA_DEFAULT_HIGHLIGHTS_PER_RESULT, EXA_DEFAULT_SEARCH_TYPE, ExaSearchProvider } from '../src/external-exa.ts'
import { EXA_PROVIDER_DEFAULTS } from '../src/config.ts'
...
maybe('ExaSearchProvider real API', () => {
  it('returns sources for a live query', async () => {
    const provider = new ExaSearchProvider(() => ({
      ...
      baseURL: process.env.EXA_BASE_URL ?? EXA_PROVIDER_DEFAULTS.baseURL,
