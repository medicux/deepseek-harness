...
Shell plugin: five-column AppFrame (drag handles and concession chain) plus the `ctx.layout` panel-geometry service; it registers into the runtime-owned `root` slot and declares `sidebar`, `workbench`, `conversation`, `details`, and `conversation.empty`. The sidebar resize boundary is an invisible hit strip, while the workbench and details boundaries retain floating pills; during concession details shrinks first and auto-closes, then the workbench shrinks toward its minimum and auto-closes, with center absorbing any remaining deficit. A closed sidebar retains a 56px control rail while the workbench and details close to zero width; the unoccupied `workbench` slot renders nothing, so the column is invisible until a feature claims it (`ctx.layout` opens and closes it). The package also seats the theme presenter: it consumes resolved `ctx.theme` snapshots and projects them onto the document (`html { color-scheme }` for native UA chrome, `body[data-ds-dark-theme]` from the active color scheme, the theme's alias tokens as inline variables on body, and one owned `<meta name="theme-color">` whose content follows the computed body background). Measuring after palette and token application keeps the rendered background as the single color authority; disposing the presenter removes its metadata node with its other global writes.
