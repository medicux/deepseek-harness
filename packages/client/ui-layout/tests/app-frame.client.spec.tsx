...
describe('AppFrame', () => {
  ...
  it('the workbench opens at its contract default, drags, and reports owner props', () => {
    const { frame, instance, slotCalls } = mountFrame()
    // Unoccupied and closed: zero track, collapsed attr, live owner props.
    expect(slotCalls.find(c => c.key === 'workbench')!.props).toEqual({ collapsed: true, width: 0 })
    act(() => { instance.actions.openWorkbench() })
    expect(tracks(frame)).toEqual([280, 400, 0])
    expect(frame.hasAttribute('data-workbench-collapsed')).toBe(false)
    const lastCall = slotCalls.filter(c => c.key === 'workbench').at(-1)!
    expect(lastCall.props).toEqual({ collapsed: false, width: 400 })
    // The workbench handle sits at sidebar+workbench and widens on +dx.
    // Details is closed here: only the sidebar and workbench handles render.
    const handles = frame.querySelectorAll('[class*="handle"]')
    expect(handles).toHaveLength(2)
    drag(handles[1]!, 680, 750)
    expect(instance.getSnapshot().workbench).toBe(470)
    act(() => { instance.actions.closeWorkbench() })
    expect(tracks(frame)).toEqual([280, 0, 0])
    expect(frame.hasAttribute('data-workbench-collapsed')).toBe(true)
  })

  it('the closed column shows an open affordance; activating it opens at the default', () => {
    const { frame, instance, rerenderFrame } = mountFrame()
    expect(tracks(frame)).toEqual([280, 0, 0])
    const toggle = frame.querySelector('[class*="workbenchToggle"]') as HTMLButtonElement
    expect(toggle).toBeTruthy()
    act(() => { toggle.click(); rerenderFrame() })
    expect(tracks(frame)).toEqual([280, 400, 0])
    // Open: the tab disappears (the handle owns the seam) ...
    expect(frame.querySelector('[class*="workbenchToggle"]')).toBeNull()
    // ... and a double-click on the handle closes the column again.
    const handles = frame.querySelectorAll('[class*="handle"]')
    act(() => { handles[1]!.dispatchEvent(new Event('dblclick', { bubbles: true })); rerenderFrame() })
    expect(tracks(frame)).toEqual([280, 0, 0])
    expect(instance.getSnapshot().details).toBe(0)
  })

  it('a squeezed viewport auto-closes only the details column while the workbench holds', () => {
    frameWidth = 1400 // 280 + 400 + 360 + 640 = 1680 > 1400
    const { frame, instance } = mountFrame()
    act(() => { instance.actions.openWorkbench() })
    act(() => { instance.actions.openDetails() })
    // Concession chain: details shrinks then closes; the workbench keeps its preference.
    expect(tracks(frame)).toEqual([280, 400, 0])
    expect(instance.getSnapshot().details).toBe(360)
  })
