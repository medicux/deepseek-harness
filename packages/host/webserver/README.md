...
Web HTTP and upgrade-route registration plugin (default-exported `WebServer`, config `{host, port, carrier}`): with the default `carrier: 'tcp'` a `node:http` server listens on activation and provides `ctx.webServer`; with `carrier: 'stdio'` nothing binds — `listening` stays false, reading `port` throws, and a supervisor drives dispatch through `serveStdio` over its pipes. `register(route)` adds a named `exact`/`prefix` HTTP route; `registerUpgrade(route)` adds an upgrade route for an exact pathname. A duplicate path within either table throws because route patterns are a composition-level contract and a collision is a misconfiguration; both methods return a disposer that removes the registration. `registerFallback(handler)` registers the one handler for requests that match no named route. A second registration throws; the SPA dist server [`dsh-host-frontend-static`](../frontend-static/README.md) is the shipped owner, and the server returns 404 while none is registered. `tapIndex(transform)` adds an index.html transform, and `applyIndexTaps(html)` runs a body through the registered transforms in order; the fallback handler calls it on every index response. `port` reads the listening port (the OS-assigned value when `port` is 0), and `host` reads the configured bind host (composition-time facts other plugins adapt to, e.g. the directory-picker chooser). HTTP match order is fixed: exact over the whole table, then longest prefix, then the fallback handler. Upgrades match exactly and unmatched connections are closed; registration order carries no request-facing semantics.

The package knows no harness concepts and serves no files: the `/api` HTTP bridge and downlink WebSockets are routes owned by the connection plugin, plugin bundles and the HMR event stream are routes owned by the modules/hmr plugins, and dist serving belongs to the fallback owner. The upgrade handler owns the protocol handshake and connection contents; the webserver only delivers the raw socket and request. `host` accepts only `127.0.0.1` (default posture) and `0.0.0.0` (deliberate network exposure). This server serves browsers only; the desktop shell loads the page over its privileged app scheme and carries fetch over an IPC bridge. For a supervisor that wants no listening socket at all, `serveStdio(server, input, output)` drives the same dispatch — routes, taps, and fallback, byte-for-byte the same handlers — over NDJSON frames on the child's stdin/stdout (`decodeRequest`/`encodeHead`/`encodeChunk`/`encodeEnd`/`encodeDestroy` in `stdio-frames.ts`; body bytes ride base64 chunks). This package never prints; the readiness line belongs to the shell.
